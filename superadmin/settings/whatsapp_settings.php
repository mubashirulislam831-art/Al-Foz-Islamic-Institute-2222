...
<div class="flex-grow flex flex-col min-h-screen bg-transparent">
  <div class="p-6 md:p-8 flex-grow">
    ...
    <!-- Active Tab Panel: WhatsApp Settings -->
    <div class="bg-white rounded-2xl border border-primary/10 p-6 md:p-8 shadow-sm">
      <h3 class="font-extrabold text-sm text-primary uppercase tracking-wider mb-6 pb-2 border-b border-primary/5">WhatsApp API Notifications</h3>
      <form onsubmit="event.preventDefault(); alert('WhatsApp Gateway integrated successfully!');" class="space-y-6 max-w-2xl text-xs">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label class="block text-[10px] uppercase font-bold tracking-wider text-primary/70 mb-2">WhatsApp Gateway Provider</label>
            <select class="w-full px-4 py-3 border border-primary/10 rounded-xl focus:ring-1 focus:ring-primary focus:outline-none">
              <option selected>Twilio WhatsApp API</option>
              <option>MessageBird Gateway</option>
              <option>Custom Webhook</option>
            </select>
          </div>
          <div>
            <label class="block text-[10px] uppercase font-bold tracking-wider text-primary/70 mb-2">WhatsApp Number (Sender)</label>
            <input type="text" value="+92 300 1234567" class="w-full px-4 py-3 border border-primary/10 rounded-xl focus:ring-1 focus:ring-primary focus:outline-none">
          </div>
          <div class="md:col-span-2">
            <label class="block text-[10px] uppercase font-bold tracking-wider text-primary/70 mb-2">API Secret Token</label>
            <input type="password" value="wh_live_key_••••••••••••••••••••••••" class="w-full px-4 py-3 border border-primary/10 rounded-xl focus:ring-1 focus:ring-primary focus:outline-none">
          </div>
        </div>
