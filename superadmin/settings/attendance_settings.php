...
<div class="flex-grow flex flex-col min-h-screen bg-transparent">
  <div class="p-6 md:p-8 flex-grow">
    ...
    <!-- Active Tab Panel: Attendance Settings -->
    <div class="bg-white rounded-2xl border border-primary/10 p-6 md:p-8 shadow-sm">
      <h3 class="font-extrabold text-sm text-primary uppercase tracking-wider mb-6 pb-2 border-b border-primary/5">Attendance Controls</h3>
      <form onsubmit="event.preventDefault(); alert('Attendance checks saved!');" class="space-y-6 max-w-2xl text-xs">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label class="block text-[10px] uppercase font-bold tracking-wider text-primary/70 mb-2">Maximum Allowed Absences (Per Term)</label>
            <input type="number" value="6" min="1" max="20" class="w-full px-4 py-3 border border-primary/10 rounded-xl focus:ring-1 focus:ring-primary focus:outline-none">
          </div>
          <div>
            <label class="block text-[10px] uppercase font-bold tracking-wider text-primary/70 mb-2">Automated Parent Alerts for Absence</label>
            <select class="w-full px-4 py-3 border border-primary/10 rounded-xl focus:ring-1 focus:ring-primary focus:outline-none">
              <option selected>SMS & WhatsApp (Immediate)</option>
              <option>Email only (End of day)</option>
